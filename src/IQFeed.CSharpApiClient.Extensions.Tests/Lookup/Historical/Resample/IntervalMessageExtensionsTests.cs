...
namespace IQFeed.CSharpApiClient.Extensions.Tests.Lookup.Historical.Resample
{
    ...
    {
        private readonly List<IntervalMessage> _intervalsAscending;
        private readonly List<IntervalMessage> _intervalsDescending;
        private readonly TimeSpan _interval = TimeSpan.FromMinutes(1);

        public IntervalMessageExtensionsTests()
        {
            _intervalsAscending = new List<IntervalMessage>()
            {
                new IntervalMessage(new DateTime(2000, 01, 01, 9, 30, 4), 1, 1, 1, 1, 1, 1, 1),
                new IntervalMessage(new DateTime(2000, 01, 01, 9, 34, 1), 1, 1, 1, 1, 2, 1, 1),
            };

            _intervalsDescending = new List<IntervalMessage>(_intervalsAscending);
