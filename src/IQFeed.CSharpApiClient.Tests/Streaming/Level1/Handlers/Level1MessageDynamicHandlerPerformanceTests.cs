...
namespace IQFeed.CSharpApiClient.Tests.Streaming.Level1.Handlers
{
    ...
    [Explicit]
    public class Level1MessageDynamicHandlerPerformanceTests
    {
        [Test]
        public void Should_Process_Update_Message_With_Standard_Fields()
        {
            // Arrange
            var level1MessageDynamicHandler = new Level1MessageDynamicHandler();
            // set the exact fields that are currently used in good old UpdateSummaryMessage
            level1MessageDynamicHandler.SetDynamicFields(
            ...
            level1MessageDynamicHandler.Update += message => { };
            ...
            for (int i = 0; i < ExecutionsCount; i++)
            {
                ...
                {
                    level1MessageDynamicHandler.ProcessMessages(msgBytes, count);
                }
                sw.Stop();
