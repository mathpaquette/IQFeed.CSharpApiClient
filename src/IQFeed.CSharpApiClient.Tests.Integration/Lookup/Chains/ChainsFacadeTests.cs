...
namespace IQFeed.CSharpApiClient.Tests.Integration.Lookup.Chains
{
    ...
    public class ChainsFacadeTests
    {
        ...
        {
            var futureMessages =
                    await _lookupClient.Chains.GetChainFutureAsync(FutureSymbol, string.Empty, Years, 4);
        ...
        {
            var futureSpreadMessages = await _lookupClient.Chains.GetChainFutureSpreadsAsync(FutureSymbol, string.Empty, Years, 4);
        ...
        {
            var futureOptionMessages = await _lookupClient.Chains.GetChainFutureOptionAsync(FutureSymbol, OptionSideFilterType.CP, string.Empty, Years, 4);
